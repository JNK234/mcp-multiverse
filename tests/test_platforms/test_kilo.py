# Tests for Kilo Code platform adapter
import json
...
def test_kilo_adapter_properties(tmp_path: Path) -> None:
    """Test adapter name and config_path property."""
    config_file = tmp_path / "mcp_settings.json"  # Different filename!
    adapter = KiloAdapter(config_path=config_file)

    assert adapter.name == "Kilo Code"
    assert adapter.config_path is None  # File doesn't exist yet


def test_kilo_load_empty(tmp_path: Path) -> None:
...
def test_kilo_load_servers(tmp_path: Path) -> None:
...
def test_kilo_save_creates_file(tmp_path: Path) -> None:
...
def test_kilo_save_adds_required_defaults(tmp_path: Path) -> None:
...
def test_kilo_save_preserves_env(tmp_path: Path) -> None:
...
def test_kilo_save_replaces_all(tmp_path: Path) -> None:
...
def test_kilo_save_updates_existing(tmp_path: Path) -> None:
...
def test_kilo_roundtrip(tmp_path: Path) -> None:
...
def test_kilo_adapter_protocol_compliance(tmp_path: Path) -> None:
    """Test KiloAdapter implements PlatformAdapter protocol correctly."""
...
def test_kilo_json_format(tmp_path: Path) -> None:
...
def test_kilo_empty_env_not_written(tmp_path: Path) -> None:
...
def test_kilo_load_ignores_vs_code_fields(tmp_path: Path) -> None:
